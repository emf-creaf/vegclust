    ...
    if(length(nb)>0) {
      ...
      if(length(nbm)>0) {
        ...
        for(i in 1:length(nbm)) {
          ...
          dfin = .distanceToSegment(Dinifin_core[med,med], Dini_core[med,nbm[i]], Dfinini_core[med,nbm[i]])
    ...
    if(length(nb)>0) {
      ...
      if(length(nbm)>0) {
        ...
        for(i in 1:length(nbm)) {
          ...
          dfin = .distanceToSegment(Dinifin_core[med,med], Dinifin_core[med,nbm[i]], Dfin_core[med,nbm[i]])
          selnb[i] = (dfin[1] == 0)
        }
        nbm = nbm[selnb]
      }    
