...
We begin by loading `vegclust` and other libraries needed in this document:
...
```
Zero angles indicate that consecutive segments are in a straight line. The larger the angle value, the more is trajectory changing in direction. Mean and standard deviation statistics of angles are calculated according to circular statistics (see R package `circular`). While site '1' has a linear trajectoy, some segments are shorter and angles are > 0 for trajectories of site '2' and '3', denoting the change in direction. In this case, the same information could be obtained by inspecting the PCoA representation, but in a case where $\Omega$ has many dimensions, the PCoA representation will correspond to a reduced space and hence, angles and lengths will not correspond exactly to those of functions `trajectoryLengths` and `trajectoryAngles`, which take into account the full space. Angles can be calculated for all four triplets of ordered community states, whether of consecutive segments or not (i.e., between points 1-2-3, 1-2-4, 1-3-4 and 2-3-4). This is done by specifying `all=TRUE` in `trajectoryAngles`:

```{r} 
trajectoryAngles(D, sites, surveys, all=TRUE)
```

One way of determining the degree to which a trajectory follows a single direction (i.e. more or less straight pathways) involves considering the angles corresponding to all pairs of ordered triplets in the trajectory and calculating the *mean resultant length* of circular statistics (column `rho` of the previous result), which takes values between 0 and 1. This approach, however, uses only angular information and does not take into account the length of segments. For this reason, we recommend using another statistic that is sensitive to both angles and segment lengths and is implemented in function `trajectoryDirectionality`:
```{r} 
...
```{r, echo=T, fig=TRUE, fig.height=6, fig.width=6, fig.align = "center"}
xret = cmdscale(Ds)
par(mar=c(4,4,1,1))
...
```{r avoca_pcoa, echo=T, fig=TRUE, fig.height=6, fig.width=6, fig.align = "center"}
...
The following code displays the relationship between the statistic in `trajectoryDirectionality` and the mean resultant vector length that uses angular information only:
...
par(mar=c(4,4,1,1))
plot(avoca_rho, avoca_dir, xlab = "rho(T)", ylab = "dir(T)", xlim=c(0.7,1), ylim= c(0.7,1), asp=1, type="n")
...
```{r avoca_DT_PCoA, echo=TRUE, fig = TRUE, fig.height=5, fig.width=6, fig.align="center"}
...
```
The length of segments and the trajectory have increased, but all segments are of the same length, in agreement with the original trajectory. In contrast, the angles are now 90 degrees and the directionality of the trajectory has decreased substantially. In sum, the square root transformation is not recommended when examining the angular properties of trajectories.
...
5. Global transformation of dissimilarities by using non-metric multidimensional scaling (non-metric MDS).
...
Finally, we also try the configuration provided by non-metric MDS, as implemented in function `isoMDS` of package `MASS`:
```{r, echo=TRUE, fig = TRUE, fig.height=5, fig.width=6, fig.align="center"}
nmds <- isoMDS(D, y=res$conf, k = length(Sj)-1)
nmdsD <- dist(nmds$points)
trajectoryPlot(nmds$points, sites, surveys, selection=1,length=0.1, axes=c(1,2))
```
...
stress0(D,nmds$points, type="interval")
```
...
anglesnmdsD = trajectoryAngles(nmdsD,sites,surveys)

df<-as.data.frame(rbind(anglesD, anglesSqrtD, anglesPcoaD, anglesmmdsD, anglesnmdsD))
row.names(df)<-c("local", "global.sqrt", "global.pcoa", "global.mmds", "global.nmds")
round(df,2)
```
...
trajectoryDirectionality(nmdsD,sites,surveys)
```
